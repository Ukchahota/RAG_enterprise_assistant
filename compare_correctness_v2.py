...
Put this file in the repo root and run:
    python compare_correctness_v2.py                     # blind S1/S3 relabels (primary analysis)
    python compare_correctness_v2.py --labels original   # original S1/S3 labels (sensitivity check)

Writes (derived, safe to regenerate):
    original -> results/correctness_all_systems.csv
    blind    -> results/correctness_all_systems_blind.csv
"""
...
S1_ORIGINAL = RES / "correctness_s1_to_label.csv"   # also the source of s3_action
LABEL_SETS = {
    "original": {
    ...
    "blind": {
        "s0": RES / "correctness_s0_to_label.csv",
        "s1": RES / "correctness_s1_relabel_blind.csv",
        "s3": RES / "correctness_s3_relabel_blind.csv",
    },
}
OUTPUTS = {
    "original": RES / "correctness_all_systems.csv",
    "blind": RES / "correctness_all_systems_blind.csv",
}
...
def main():
    ...
    ap.add_argument("--labels", choices=["original", "blind"], default="blind")
    mode = ap.parse_args().labels
    files = LABEL_SETS[mode]

    print(f"LABEL SET: {mode}")
    for s, p in files.items():
        print(f"  {s.upper()}: {p.name}")

    data = {s: load(s, p) for s, p in files.items()}

    if mode == "blind":
        s3_orig = load("s3 original", LABEL_SETS["original"]["s3"])
        was_refused = set(s3_orig.loc[s3_orig["correctness"] == "refused", "question_id"])
        miskeyed = data["s3"][(data["s3"]["correctness"] == "incorrect")
                              & data["s3"]["question_id"].isin(was_refused)]
        if len(miskeyed):
            sys.exit(f"S3 blind sheet still has {len(miskeyed)} mis-keyed refusal(s) labelled 'incorrect'. "
                     "Run: python recode_s3_refusals.py  then rerun this script.")
    ...
    s0 = data["s0"]
    s1_orig = pd.read_csv(S1_ORIGINAL, dtype=str, keep_default_na=False)
    s1_orig["question_id"] = s1_orig["question_id"].str.strip()
    merged = s0[["question_id", "question_type", "difficulty"]].copy()
    merged = merged.merge(s1_orig[["question_id", "s3_action"]], on="question_id", how="left")
    for s in SYSTEMS:
        merged = merged.merge(
