...
def main() -> None:
    ...
    for _, question_row in tqdm(questions_df.iterrows(), total=len(questions_df)):
        ...
        row = {
            ...
            "rewritten_query": results.iloc[0]["rewritten_query"],
            "rewrite_applied": results.iloc[0]["rewrite_applied"],
            ...
            "original_ranks": safe_join(results["original_rank"]),
            "rewritten_ranks": safe_join(results["rewritten_rank"]),
            "metadata_boosts": " | ".join(
                results["metadata_boost"].round(6).astype(str).tolist()
            ),
    ...
    print("\nQuery-Rewritten Retriever Evaluation")
    print("=" * 50)
    print("Retriever: Corrective query rewriting + Hybrid FAISS/BM25")
    print("Fusion method: Original + rewritten query RRF")
